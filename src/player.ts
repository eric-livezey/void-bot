...
const DEFAULT_CREATE_AUDIO_RESOURCE_OPTIONS = Object.freeze({
    inlineVolume: true
...
export class Track<M = null> {
    ...
    public static fromVideoInfo(info: YT.VideoInfo): Track {
        ...
        const prepare = _createYtDlpPrepare(videoId);
     ...
     * 
     * **NOTE**: This may have have different metadata than its YouTube video equivalent.
     *
...
function _createYtDlpPrepare(videoId: string, download = SHOULD_DOWNLOAD): PrepareFunction {
    if (download) {
        return _createDownloadPrepare(videoId, path => ytdlp.download(videoId, path, MAX_RETRIES));
    } else {
        return _createStreamPrepare(() => ytdlp.createStream(videoId, MAX_RETRIES));
    }
}

function _createDownloadPrepare(id: string, fn: (path: string) => Promise<void>): PrepareFunction {
    ...
    return async function prepare() {
        if (existsSync(file)) {
            return createAudioResource(file, DEFAULT_CREATE_AUDIO_RESOURCE_OPTIONS);
        } else {
            return fn(file).then(() => createAudioResource(file, DEFAULT_CREATE_AUDIO_RESOURCE_OPTIONS));
        }
    }
}

function _createStreamPrepare(fn: () => Promise<Readable> | Readable): PrepareFunction {
    return async function () {
        return createAudioResource(await fn(), DEFAULT_CREATE_AUDIO_RESOURCE_OPTIONS);
    }
}
